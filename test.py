...
def change_advertisement_status(account):
    try:
        ...
        contract.functions.updateAdStatus(idAd).transact({'from': account})
...
def get_ads():
    try:
        ...
        for ad in ads:
            print(f"ID объявления: {ad[4]}, ID недвижимости: {ad[3]}, Цена: {ad[2]}, Владелец: {ad[0]}, Покупатель: {ad[1]}, Статус: {ad[6]}")
    except Exception as e:
